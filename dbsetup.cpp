...
database::mariadb::MariaDBClient DBSetup::InitDB(){
    ...
    mariaDbClient.exec(
                "CREATE TABLE IF NOT EXISTS `threads` (thread_id int(11) auto_increment, title varchar(255), content varchar(255), created_at timestamp default current_timestamp, userid int(11), primary key(thread_id), foreign key(userid) references users(user_id));",
                [&mariaDbClient](void) -> void {
        VLOG(0) << "**** OnQuery 0;";
        mariaDbClient.affectedRows(
                    [](my_ulonglong affectedRows) -> void {
    ...
    mariaDbClient.exec(
                "CREATE TABLE IF NOT EXISTS `posts` (post_id int(11) auto_increment, content varchar(255), created_at timestamp default current_timestamp, userid int(11), primary key(post_id), foreign key(userid) references users(user_id));",
                [&mariaDbClient](void) -> void {
        VLOG(0) << "**** OnQuery 0;";
        mariaDbClient.affectedRows(
                    [](my_ulonglong affectedRows) -> void {
